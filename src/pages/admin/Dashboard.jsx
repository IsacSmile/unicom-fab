...
import { Package, ShoppingCart, MessageSquare, AlertTriangle, TrendingUp, Sparkles, DollarSign } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="font-display font-bold text-3xl text-brand-950 mt-1">{stats.totalProducts}</div>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="font-display font-bold text-3xl text-brand-950 mt-1">{stats.totalOrders}</div>
                  <span className="text-xs text-amber-700 font-bold">{stats.pendingOrders} Pending Action</span>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="font-display font-bold text-3xl text-brand-950 mt-1">{stats.totalEnquiries}</div>
                  <span className="text-xs text-emerald-700 font-bold">{stats.newEnquiries} New Leads</span>
                </div>
                ...
                <div>
                  ...
                  <div className="text-sm font-bold text-brand-950 mt-2 space-y-1">
                    <div className="flex items-center gap-1.5 text-amber-800">
                      <TrendingUp className="w-4 h-4" /> {stats.trendingProducts} Trending Lines
                    </div>
                    <div className="flex items-center gap-1.5 text-emerald-800">
                      <Sparkles className="w-4 h-4" /> {stats.newArrivalProducts} New Arrivals
                    </div>
                  </div>
                </div>
              </div>
            ...
            {stats.lowStockProducts && stats.lowStockProducts.length > 0 && (
              <div className="bg-amber-50 border border-amber-300 p-6 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 text-amber-900 font-bold text-base">
                  ...
                  <span>Low Stock Alert ({stats.lowStockProducts.length} Items Below MOQ / Threshold)</span>
                </div>
