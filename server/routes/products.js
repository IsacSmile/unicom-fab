...
async function formatProduct(p) {
  if (!p) return null;
  const rawImages = await db.prepare('SELECT image_url FROM product_images WHERE product_id = ? ORDER BY display_order ASC').all(p.id);
  const rawColours = await db.prepare('SELECT colour_name FROM product_colours WHERE product_id = ?').all(p.id);
  const rawSizes = await db.prepare('SELECT size_name FROM product_sizes WHERE product_id = ?').all(p.id);

  const images = (rawImages || []).map(i => i.image_url);
  const colours = (rawColours || []).map(c => c.colour_name);
  const sizes = (rawSizes || []).map(s => s.size_name);
...
router.get('/', async (req, res) => {
  try {
    ...
    const products = await Promise.all((rows || []).map(p => formatProduct(p)));
    ...
    const totalCountResult = await db.prepare(countQuery).get(...params);
...
router.get('/meta/filters', async (req, res) => {
  try {
    const rawCategories = await db.prepare('SELECT DISTINCT category FROM products WHERE category IS NOT NULL ORDER BY category ASC').all();
    const rawColours = await db.prepare('SELECT DISTINCT colour_name FROM product_colours ORDER BY colour_name ASC').all();
    const rawSizes = await db.prepare('SELECT DISTINCT size_name FROM product_sizes ORDER BY size_name ASC').all();
    ...
    console.error('Error in GET /api/products/meta/filters:', err);
...
router.get('/:idOrSlug', async (req, res) => {
  try {
    ...
    const p = await db.prepare('SELECT * FROM products WHERE id = ? OR slug = ?').get(idOrSlug, idOrSlug);

    if (!p) {
      return res.status(404).json({ error: 'Product not found' });
    }

    const formatted = await formatProduct(p);
    return res.json({ product: formatted });
  } catch (err) {
    console.error('Error in GET /api/products/:idOrSlug:', err);
